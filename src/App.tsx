...
import {
  Pause, Play, FileDown, XCircle, Trash2, ChevronsUp, MoreVertical, Check, X, Info, RotateCcw
} from 'lucide-react'
...
function ConfirmModal({
  open, title, body, requireText = 'confirm', onClose, onConfirm
}:{
  open: boolean;
  title: string;
  body?: string;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative z-10 w-[min(92vw,520px)] rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-4 shadow-xl">
        ...
        <div className="text-xs mb-2 text-slate-500">
          Type <span className="font-mono px-1 py-0.5 rounded bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700">confirm</span> to continue
        </div>
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100">
      ...
      <div className="container py-6 space-y-6">
        ...
        <QueueTable
          ...
          onReset={async (id)=> {
            try { await api.reset(id); pushToast({type:'success', title:'Job reset'}) }
            catch(e:any){ pushToast({type:'error', title:'Reset failed', detail:String(e?.message || e)}) }
          }}
        />
      </div>
    </div>
...
function UploadCard({
  ...
  // Validate file + build mapping preview (uses targetName || targetHomepage)
  useEffect(() => {
...
function QueueTable({
  ...
  onReorder, onPause, onResume, onExport, onCancel, onDelete, onReset,
  pushToast,
}:{
  jobs: Job[];
  isFetching: boolean;
  ...
  // DnD on current page (reorder full list)
  ...
  // bulk actions with confirm for cancel/delete/reset if needed (here only cancel uses confirm in menu)
  const doBulk = async (kind:'pause'|'resume'|'cancel') => {
    const can = (j:Job) => kind==='pause' ? ['running', 'queued'].includes(j.status)
                  : kind==='resume' ? ['paused','pausing'].includes(j.status)
                  : j.status==='running'
  ...
  const pageButtons = React.useMemo<(number|'dots')[]>(() => {
    ...
    set.add(1); set.add(total); set.add(cur); set.add(cur+1); set.add(cur+2)
  ...
  const askConfirm = (title:string, body:string, action:()=>Promise<void>) => {
    setConfirm({
      ...
      title, body,
      onConfirm: async () => {
        try { await action() } finally { setConfirm(c=>({ ...c, open:false })) }
      }
    })
  }
  ...
  return (
    <div className="card p-3">
      ...
      {/* Row 2: Owners + Statuses side-by-side */}
            ...
            <SortableContext items={pageIds} strategy={verticalListSortingStrategy}>
              <tbody>
                {paged.map((j, idxOnPage) => {
                  ...
                  const createdAt = (j as any).created_at as string | undefined
                        ...
                        <>
                          ...
                          <td className="align-top">
                            <RowActions
                              ...
                              onDelete={()=>askConfirm('Delete job?', `This will permanently remove the job and its results.\n\nJob: ${j.name}`, () => onDelete(j.id))}
                              onReset={()=>askConfirm('Reset job?', `This will set the job status to "queued" and all row states to "pending".\n\nJob: ${j.name}`, () => onReset(j.id))}
      ...
      <div className="mt-3 flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          {safePage < pageCount && (
            <button className="btn" onClick={()=>setPage(p=>Math.min(pageCount, p+1))}>Next</button>
          )}
...
function RowActions({
  job, onResume, onPause, onCancel, onDelete, onExport, onTop, onUp, onDown, onReset
}:{
  job: Job
  onResume: ()=>void
  onPause: ()=>void
  ...
  // decide dropUp/dropDown on open
  React.useEffect(() => {
  ...
  const Menu = (
    <div className={clsx(
      "absolute right-0 w-48 rounded-md border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-lg z-20",
      ...
      <ul className="py-1 text-sm">
        ...
        <li><button className="menu-item" onClick={()=>{ setOpen(false); onReset() }}>
          <RotateCcw size={14} className="mr-2"/> Reset job
        </button></li>

        <li><button disabled={job.status!=='running'} className="menu-item text-amber-700 disabled:opacity-50" onClick={()=>{ setOpen(false); onCancel() }}>
